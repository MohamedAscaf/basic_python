...
def updatec(amt,id,win):
    connection = mysql.connect(host="localhost",user="root",password="livewire",database="PythonBank")
...
def customerpage(cid,win):
    ...
    x = Label(frame, text="SANRIA BANK", font=label_font, bg="Red")
...
def customer(win):
    ...
    frame.pack()
    label_font=font.Font(weight="bold",family="Times New Roman",size=30)
    x=Label(frame,text="SANRIA BANK",font=label_font)
    x.config(bg= "blue", fg= "white")
    x.place(relx=0.43,rely=0.01)
...
frame.place(relx=0.0,rely=0.94)
label_font=font.Font(weight="bold",family="Times New Roman",size=20)
x=Label(frame,text="Copyright © 2023 SANRIA BANK ",font=label_font,bg="Red")
x.config(bg= "blue", fg= "white")
x.place(relx=0.35,rely=0.1)
win.mainloop()
